...
**Last Minute Student** é um *endless runner* 3D em perspetiva na terceira pessoa. O jogador controla um estudante que corre para chegar à aula a tempo, enquanto é perseguido pelo trabalhador de contucao civil apos ter pisado o cimento fresco. O objetivo é sobreviver o máximo de tempo possível, desviando de obstáculos e recolhendo colecionáveis que conferem pontos e poderes.
...
- **Salto** com física personalizada (velocidade inicial 12 m/s, gravidade 26 m/s²).
- **Perseguidor (bob)** com gestão de distância dinâmica: o chaser aproxima-se ao longo do tempo e desacelera quando o jogador acerta num obstáculo; ao atingir o jogador, aciona uma cinemática onde apanmha o player.
- **Obstáculos variados**: barreiras, cones de trânsito, bancos, caixotes, mesas, cacifos, entre outros.
- **Colecionáveis**: livros/moedas (pontos), *Speed Boost* (velocidade ×1,5 durante 5 s).
...
- **Barra de proximidade** no HUD que indica o perigo de ser apanhado (vermelho).
...
- **UI totalmente procedural** (sem prefabs de UI): menu principal, HUD, ecrã de *game over* e painel de definiçõe.
...
### Objetivo
Correr o maior tempo/distância possível sem ser apanhado pelo chaser. A pontuação aumenta com o tempo de sobrevivência e com os livros recolhidos.
...
### Regras
- O jogador perde se o chaser(bob construtor civil) o alcançar (distância < 1,35 unidades → cinemática de fim).
- O jogador também perde se cair para Y < -3 (fora da faixa).
- Ao colidir com um obstáculo, ocorre um *stumble* (~1 s de vulnerabilidade) e o chaser(bob construtor civil) recupera terreno.
...
| Personagem chaser(bob construtor civil) (`Ch17_nonPBR@Fast Run.fbx`) | Mixamo (free) | Baixa-média poligonagem | Distingue visualmente o perseguidor |
...
| Personagem chaser(bob construtor civil) (`Ch17_1001_Diffuse.png`) | PNG | 2048 × 2048 | Maior detalhe necessário pelo nível de zoom da câmara |
